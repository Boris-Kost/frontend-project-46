...
test('gendiff unsupported extension', () => {
  expect(() => genDiff(getFixturePath('unsupported.txt'), getFixturePath('unsupported.txt'))).toThrow('Unknown format: \'txt\'!')
})

test('gendiff deep nested object stringify', () => {
  const result = genDiff(getFixturePath('file3_nested.json'), getFixturePath('file3_nested.json'))
